...
<div class="mind-map-form w-50 mx-auto p-2 border rounded">
  <h1 class="h3 mt-2 mb-3 pb-1 font-weight-normal text-center border-bottom">ブランチ編集</h1>
      <form class="w-100 mx-auto" action="" method="POST">
        ...
        <h3 class="text"><?=h($value['m2content'])?></h3>
        <input class="form-control  my-5" id="content" type="text" name="content" value="<?=h($value['m3content'])?>" placeholder="<?=h($value['m3content'])?>" required autofocus/>
        <?php endforeach;?>
